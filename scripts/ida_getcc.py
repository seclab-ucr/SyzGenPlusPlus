...
preclassNameToParentNameMap = {"OSObject": "", "IOService": "IORegistryEntry", "IORegistryEntry": "OSObject", "IOUserClient": "IOService",
                               "OSDictionary": "OSObject", "OSArray": "OSObject", "OSSet": "OSObject", "IOMemoryDescriptor": "OSObject", "OSSymbol": "OSObject"}
...
def parseDemangledFuncNameToGetClassNameAndArglist(demangledFuncName, isVirtual):
    ...
    if demangledFuncName != None:
        ...
        if demangledFuncName.find("(") < demangledFuncName.rfind(")"):
            arglist.extend(demangledFuncName[demangledFuncName.find(
                "(")+1:demangledFuncName.rfind(")")].split(","))
...
def parseVTable(vtableStartEA, demangledClassName):
    ...
    ret = SetType(GetMemberId(classStrucId, 0),
                  "struct " + vtableStrucName + " *")
    ...
    while True:
        ...
        if not alreadyExist:
            ...
            AddStrucMember(vtableStrucId, "member" +
                           str(memberOffset/8), memberOffset, qwrdflag(), -1, 8)
        ...
        if funcType == None:
            ...
            if funcType == None:
                nouse, arglist = parseDemangledFuncNameToGetClassNameAndArglist(
                    demangledFuncName, True)
        ...
        if funcType != None:
            ...
            funcPTRType = funcType[:funcTypeArgStartLoc] + \
                "(*)" + funcType[funcTypeArgStartLoc:]
...
def createClassStruct(className, classSize):
    ...
    for memberOffset in range(currentClassSize, classSizeWithIndivMembers, 8):
        AddStrucMember(classStrucId, "member" +
                       str(memberOffset/8), memberOffset, qwrdflag(), -1, 8)
    if classSize > classSizeWithIndivMembers:
        AddStrucMember(classStrucId, "members", classSizeWithIndivMembers,
                       qwrdflag(), -1, classSize - classSizeWithIndivMembers)
    set_struc_hidden(get_struc(classStrucId), 1)
    return classStrucId
                ...
                if opertor == "call" and (opnd0 == "__ZN11OSMetaClassC2EPKcPKS_j" or opnd0 == "OSMetaClass::OSMetaClass(char const*,OSMetaClass const*,uint)"):
                    ...
                    if value != None:
                        ...
                        if valueName == None or valueName.startswith("off_"):
                            classParentMetaClass = Demangle(
                                Name(Qword(value)), GetLongPrm(INF_SHORT_DN))
                            if classParentMetaClass:
                                classParentClass = classParentMetaClass[:classParentMetaClass.rfind(
                                    "::")]
                        else:
                            classParentMetaClass = Demangle(
                                valueName, GetLongPrm(INF_SHORT_DN))
                            if classParentClass:
                                classParentClass = classParentMetaClass[:classParentMetaClass.rfind(
                                    "::")]
...
def processFuncArgs(funcEA, isVirtual, className):
    funcName = Name(funcEA)
    demangledFuncName = Demangle(funcName, GetLongPrm(INF_LONG_DN))
    classNameInFuncName, arglist = parseDemangledFuncNameToGetClassNameAndArglist(
        demangledFuncName, isVirtual)
    if (isVirtual and classNameInFuncName == className) or ((not isVirtual) and classNameInFuncName != None):
        ...
        demangledFuncNameWithoutArgs = demangledFuncName[:demangledFuncName.find(
            "(")].strip()
        demangledFuncNameWithoutArgs = demangledFuncNameWithoutArgs.replace(
            ":", "_")
        demangledFuncNameWithoutArgs = demangledFuncNameWithoutArgs.replace(
            "~", "_")
        funcTypeToSet = funcType[:funcTypeArgStartLoc] + \
            demangledFuncNameWithoutArgs + funcType[funcTypeArgStartLoc:]
