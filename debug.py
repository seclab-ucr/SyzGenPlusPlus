...
class LLDBDebugger(object):
    ...
    def read_task(self, name, **kwargs):
        ...
        print("output from showtask: %s" % output)
        for line in output.split("\n")[1:]:
            cols = line.split()
    ...
    def write_memory(self, address, value, *args, **kwargs):
        ...
        error = lldb.SBError()
        process = lldb.target.GetProcess()
        new_value = str(value)  # FIXME: python2 style
    ...
    def find_symbol_addr(self, name):
        symbol = lldb.target.FindSymbols(name)
        return [
            { "addr": each.addr.load_addr }
            for each in symbol.symbols
        ]
    ...
    def find_section_name(self, addr):
        return lldb.target.ResolveLoadAddress(addr).section.name
    ...
    def set_breakpoint(self, kext, offset, target="poc"):
        output = run_cmd("showallkexts")
        for line in output.split("\n")[1:]:
            cols = line.split()
            if len(cols) != 10:
            ...
            if name == kext or name.endswith(kext):
                ...
                cmd = "br set -a 0x%x" % (addr + offset)
                # See ShowCurrentThreads at /Library/Developer/KDKs/KDK_10.15.4_19E287.kdk/
                # System/Library/Kernels/kernel.development.dSYM/Contents/Resources/Python//lldbmacros/process.py
                conditions = [
                    "((proc*)processor_list->active_thread->task->bsd_info)->p_comm[%d] == %d" % (i, ord(c))
                    for i, c in enumerate(target + "\x00")
                ]
                cmd += " -c \'"
                cmd += " && ".join(conditions)
                cmd += "\'"
...
class Proxy(LLDBCommand):
    ...
    def connect(self):
        ...
        self.sock.connect(('localhost', int(os.getenv("PROXY_PORT"))))
    ...
    def read_register(self, request):
        ret = {}
        reg_name = str(request["reg"])  # FIXME: python2 style
    ...
    def find_symbols(self, request):
        names = request["names"]
        symbols = {}
        for name in names:
            try:
                name = str(name)
                symbols[name] = self.debugger.find_symbol_addr(name)
            except Exception as e:
                print(e)
                symbols[name] = []
        return {"errcode": 0, "symbols": symbols}
    ...
    @catch_exception
    def find_section_name(self, request):
        name = self.debugger.find_section_name(request["addr"])
        if name is None: name = ""
        return {"name": name}
    ...
    def showtask(self, request):
        ret = {}
        name = str(request["name"])  # FIXME: python2 style
    ...
    def setTarget(self, request):
        self.target = str(request["target"])  # FIXME: python2 style
    ...
    def set_breakpoint(self, request):
        self.debugger.set_breakpoint(request["kext"], request["addr"], request["target"])
    ...
    def handle_command(self, request):
        ...
        if cmd == "read reg":
        ...
        elif cmd == "find symbol":
            self.send(self.find_symbols(request))
        elif cmd == "find name":
            self.send(self.find_function_name(request))
        elif cmd == "find section":
            self.send(self.find_section_name(request))
        elif cmd == "find var":
            ...
            self.send(self.handle_output(**request))
        else:
